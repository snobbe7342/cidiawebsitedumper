...
function initUserScriptHandler() {
	addEventListener("single-file-user-script-init", () => globalThis._CidiaWebsiteDump_waitForUserScript = async eventPrefixName => {
...
function postProcessDoc(doc, markedElements) {
	...
	if (!markedElements) {
		const CidiaWebsiteDumpAttributes = [REMOVED_CONTENT_ATTRIBUTE_NAME, HIDDEN_FRAME_ATTRIBUTE_NAME, HIDDEN_CONTENT_ATTRIBUTE_NAME, PRESERVED_SPACE_ELEMENT_ATTRIBUTE_NAME, IMAGE_ATTRIBUTE_NAME, POSTER_ATTRIBUTE_NAME, CANVAS_ATTRIBUTE_NAME, INPUT_VALUE_ATTRIBUTE_NAME, SHADOW_ROOT_ATTRIBUTE_NAME, HTML_IMPORT_ATTRIBUTE_NAME, STYLESHEET_ATTRIBUTE_NAME, ASYNC_SCRIPT_ATTRIBUTE_NAME];
		markedElements = doc.querySelectorAll(CidiaWebsiteDumpAttributes.map(name => "[" + name + "]").join(","));
	}
