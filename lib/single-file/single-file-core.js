...
function getClass(...args) {
	[util, cssTree] = args;
	return CidiaWebsiteDumpClass;
}

class CidiaWebsiteDumpClass {
	constructor(options) {
...
class Processor {
	...
	async getPageData() {
		...
		if (this.options.insertCidiaWebsiteDumpComment) {
					...
					if (imageData) {
						if (this.options.removeHiddenElements && (
							...
							if (imageData.currentSrc) {
								imgElement.dataset.CidiaWebsiteDumpOriginURL = imgElement.getAttribute("src");
	...
	async resolveFrameURLs() {
		...
		async function initializeProcessor(frameData, frameElement, frameWindowId, batchRequest, options) {
			options.insertCidiaWebsiteDumpComment = false;
...
class ProcessorHelper {
	...
	static async processAttribute(resourceElements, attributeName, baseURI, options, cssVariables, styles, batchRequest, processDuplicates, removeElementIfMissing) {
		await Promise.all(Array.from(resourceElements).map(async resourceElement => {
			...
			if (resourceURL != null) {
				resourceURL = normalizeURL(resourceURL);
				let originURL = resourceElement.dataset.CidiaWebsiteDumpOriginURL;
				delete resourceElement.dataset.CidiaWebsiteDumpOriginURL;
				if (!testIgnoredPath(resourceURL)) {
