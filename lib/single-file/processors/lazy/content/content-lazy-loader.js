...
if (browser && browser.runtime && browser.runtime.onMessage && browser.runtime.onMessage.addListener) {
	browser.runtime.onMessage.addListener(message => {
		if (message.method == "CidiaWebsiteDump.lazyTimeout.onTimeout") {
...
async function setAsyncTimeout(type, callback, delay) {
	if (browser && browser.runtime && browser.runtime.sendMessage) {
		if (!timeouts.get(type) || !timeouts.get(type).pending) {
			...
			try {
				await browser.runtime.sendMessage({ method: "CidiaWebsiteDump.lazyTimeout.setTimeout", type, delay });
			} catch (error) {
...
async function clearAsyncTimeout(type) {
	if (browser && browser.runtime && browser.runtime.sendMessage) {
		try {
			await browser.runtime.sendMessage({ method: "CidiaWebsiteDump.lazyTimeout.clearTimeout", type });
		} catch (error) {
