...
const INIT_REQUEST_MESSAGE = "CidiaWebsiteDump.frameTree.initRequest";
const ACK_INIT_REQUEST_MESSAGE = "CidiaWebsiteDump.frameTree.ackInitRequest";
const CLEANUP_REQUEST_MESSAGE = "CidiaWebsiteDump.frameTree.cleanupRequest";
const INIT_RESPONSE_MESSAGE = "CidiaWebsiteDump.frameTree.initResponse";
const TARGET_ORIGIN = "*";
...
function initRequestSync(message) {
	...
	const waitForUserScript = globalThis._CidiaWebsiteDump_waitForUserScript;
	if (!TOP_WINDOW) {
...
function sendInitResponse(message) {
	...
	try {
		top.CidiaWebsiteDump.processors.frameTree.initResponse(message);
	} catch (error) {
