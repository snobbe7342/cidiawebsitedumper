...
browser.runtime.onMessage.addListener(message => {
	if (message.method == "CidiaWebsiteDump.fetchFrame" && window.frameId && window.frameId == message.frameId) {
...
async function fetchResource(url, options = {}) {
	try {
		...
		const response = await sendMessage({ method: "CidiaWebsiteDump.fetch", url, referrer: options.referrer });
		return {
			status: response.status,
...
async function frameFetch(url, options) {
	const response = await sendMessage({ method: "CidiaWebsiteDump.fetchFrame", url, frameId: options.frameId, referrer: options.referrer });
	return {
		status: response.status,
