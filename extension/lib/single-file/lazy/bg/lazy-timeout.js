...
browser.runtime.onMessage.addListener((message, sender) => {
	if (message.method == "CidiaWebsiteDump.lazyTimeout.setTimeout") {
		...
		const timeoutId = setTimeout(async () => {
			try {
				...
				await browser.tabs.sendMessage(sender.tab.id, { method: "CidiaWebsiteDump.lazyTimeout.onTimeout", type: message.type });
			} catch (error) {
				// ignored
			}
		}, message.delay);
	...
	if (message.method == "CidiaWebsiteDump.lazyTimeout.clearTimeout") {
		let tabTimeouts = timeouts.get(sender.tab.id);
		if (tabTimeouts) {
			const frameTimeouts = tabTimeouts.get(sender.frameId);
			if (frameTimeouts) {
