...
const bootstrap = globalThis.CidiaWebsiteDumpBootstrap;

const MOZ_EXTENSION_PROTOCOL = "moz-extension:";

let processor, processing;

CidiaWebsiteDump.init({ fetch, frameFetch });
	...
	if (!location.href.startsWith(MOZ_EXTENSION_PROTOCOL)) {
		...
		if (message.method == "content.cancelSave") {
			...
			if (message.options.loadDeferredImages) {
				CidiaWebsiteDump.processors.lazy.resetZoomLevel(message.options);
			}
			return {};
		}
...
async function processPage(options) {
	const frames = CidiaWebsiteDump.processors.frameTree;
	let framesSessionId;
	CidiaWebsiteDump.helper.initDoc(document);
	ui.onStartPage(options);
	processor = new CidiaWebsiteDump.CidiaWebsiteDump(options);
	...
	if (!options.saveRawPage) {
		...
		if (options.loadDeferredImages) {
			const lazyLoadPromise = CidiaWebsiteDump.processors.lazy.process(options);
	...
	options.onprogress = event => {
		if (!processor.cancelled) {
			if (event.type == event.RESOURCES_INITIALIZED) {
				...
				if (options.loadDeferredImages) {
					CidiaWebsiteDump.processors.lazy.resetZoomLevel(options);
				}
			}
	...
	if (!processor.cancelled) {
		...
		if (options.displayStats) {
			console.log("CidiaWebsiteDump stats"); // eslint-disable-line no-console
