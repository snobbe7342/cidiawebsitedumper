...
const CidiaWebsiteDump = globalThis.CidiaWebsiteDumpBootstrap;
...
async function onMessage(message) {
	...
	if (message.method == "devtools.resourceCommitted") {
		CidiaWebsiteDump.pageInfo.updatedResources[message.url] = { content: message.content, type: message.type, encoding: message.encoding };
		return {};
	}
	if (message.method == "common.promptValueRequest") {
		browser.runtime.sendMessage({ method: "tabs.promptValueResponse", value: prompt("CidiaWebsiteDump: " + message.promptMessage) });
		return {};
	}
}

function init() {
	if (previousLocationHref != location.href && !CidiaWebsiteDump.pageInfo.processing) {
		pageAutoSaved = false;
...
async function autoSavePage() {
	const helper = CidiaWebsiteDump.helper;
	if ((!autoSavingPage || autoSaveTimeout) && !pageAutoSaved) {
		...
		if (options.autoSaveDelay && !autoSaveTimeout) {
			...
			const waitForUserScript = window._CidiaWebsiteDump_waitForUserScript;
			let frames = [];
			let framesSessionId;
			autoSaveTimeout = null;
			if (!options.removeFrames && globalThis.frames && globalThis.frames.length) {
				frames = await CidiaWebsiteDump.processors.frameTree.getAsync(options);
			}
			...
			if (framesSessionId) {
				CidiaWebsiteDump.processors.frameTree.cleanup(framesSessionId);
			}
...
function autoSaveUnloadedPage({ autoSaveUnload, autoSaveDiscard, autoSaveRemove }) {
	...
	let frames = [];
	if (!options.removeFrames && globalThis.frames && globalThis.frames.length) {
		frames = CidiaWebsiteDump.processors.frameTree.getSync(options);
	}
	if (options.userScriptEnabled && waitForUserScript) {
...
function savePage(docData, frames, { autoSaveUnload, autoSaveDiscard, autoSaveRemove } = {}) {
	const helper = CidiaWebsiteDump.helper;
	const updatedResources = CidiaWebsiteDump.pageInfo.updatedResources;
	const visitDate = CidiaWebsiteDump.pageInfo.visitDate.getTime();
...
async function openEditor(document) {
	const infobarElement = document.querySelector("CidiaWebsiteDump-infobar");
	if (infobarElement) {
		infobarElement.remove();
	}
	serializeShadowRoots(document);
	const content = CidiaWebsiteDump.helper.serialize(document);
...
function serializeShadowRoots(node) {
	...
	node.querySelectorAll("*").forEach(element => {
		const shadowRoot = CidiaWebsiteDump.helper.getShadowRoot(element);
		if (shadowRoot) {
