...
(globalThis => {
	...
	const CidiaWebsiteDump = globalThis.CidiaWebsiteDump;
	...
	const COMMENT_HEADER = "Page saved with CidiaWebsiteDump";
	const COMMENT_HEADER_LEGACY = "Archive processed by CidiaWebsiteDump";
...
.reader-show-element {
  display: initial;
		...
		if (removeHighlightMode) {
			...
			while (element && !done) {
				if (element.classList.contains(HIGHLIGHT_CLASS)) {
					document.querySelectorAll("." + HIGHLIGHT_CLASS + "[data-CidiaWebsiteDump-highlight-id=" + JSON.stringify(element.dataset.CidiaWebsiteDumpHighlightId) + "]").forEach(highlightedElement => {
	...
	function highlightSelection() {
		let highlightId = 0;
		document.querySelectorAll("." + HIGHLIGHT_CLASS).forEach(highlightedElement => highlightId = Math.max(highlightId, highlightedElement.dataset.CidiaWebsiteDumpHighlightId));
		highlightId++;
		...
		function highlightNode(node) {
			if (node.nodeType == node.ELEMENT_NODE) {
				...
				node.dataset.CidiaWebsiteDumpHighlightId = highlightId;
		...
		function highlightTextNode(node) {
			...
			spanElement.dataset.CidiaWebsiteDumpHighlightId = highlightId;
	...
	function formatPage(applySystemTheme) {
		...
		document.querySelectorAll(NOTE_TAGNAME).forEach(containerElement => {
			...
			const className = "CidiaWebsiteDump-note-id-" + containerElement.dataset.noteId;
		...
		document.querySelectorAll(NOTE_TAGNAME).forEach(containerElement => {
			const noteId = (Array.from(containerElement.classList).find(className => /CidiaWebsiteDump-note-id-\d+/.test(className))).split("CidiaWebsiteDump-note-id-")[1];
			containerElement.classList.remove("CidiaWebsiteDump-note-id-" + noteId);
			containerElement.dataset.noteId = noteId;
	...
	function getContent(compressHTML, updatedResources) {
		...
		return CidiaWebsiteDump.helper.serialize(doc, compressHTML);
	}
	...
	function resetHighlightedElement(element) {
		...
		delete element.dataset.CidiaWebsiteDumpHighlightId;
	}
	...
	function deleteNoteRef(containerElement, noteId) {
		...
		if (noteRefs.length) {
			...
			delete anchorElement.dataset.CidiaWebsiteDumpNoteRefs;
		}
	}

	function getNoteRefs(anchorElement) {
		return JSON.parse("[" + (anchorElement.dataset.CidiaWebsiteDumpNoteRefs || "") + "]");
	}

	function setNoteRefs(anchorElement, noteRefs) {
		anchorElement.dataset.CidiaWebsiteDumpNoteRefs = noteRefs.toString();
	}
