...
(globalThis => {
	...
	const INFOBAR_TAGNAME = "CidiaWebsiteDump-infobar";
	...
	const CidiaWebsiteDump_COMMENT = "CidiaWebsiteDump";
	...
	async function displayIcon() {
		...
		let CidiaWebsiteDumpComment = result && result.singleNodeValue;
		if (CidiaWebsiteDumpComment && isCidiaWebsiteDumpComment(CidiaWebsiteDumpComment)) {
			const info = CidiaWebsiteDumpComment.textContent.split("\n");
	...
	function isCidiaWebsiteDumpComment(node) {
		return node.nodeType == Node.COMMENT_NODE && node.textContent.includes(CidiaWebsiteDump_COMMENT);
	}
